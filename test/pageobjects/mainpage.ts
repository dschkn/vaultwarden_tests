...
export class MainPage extends Page {
  ...
  async scrollAndSearchByVariable(generatedName: string) {
    const itemSelector = this.getItemSelectorByTitle(generatedName);
  ...
  waitForRowElementIsDisplayed(generatedName: string) {
    return $(`//tr[td//text()[contains(., '${generatedName}')]]`);
  }
 
  async waitForButtonElementIsDisplayedAndClick(generatedName: string) {
    const rowElement = this.waitForRowElementIsDisplayed(generatedName);
